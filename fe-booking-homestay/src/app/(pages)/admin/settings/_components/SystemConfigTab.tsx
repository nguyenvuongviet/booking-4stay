    ...
    <div className="space-y-6 max-w-3xl animate-in fade-in-50 duration-200">
      <div className="flex justify-between items-center bg-primary/5 p-4 rounded-xl border border-primary/20">
        ...
        <Button
          ...
          className="gap-2 rounded-lg cursor-pointer bg-card hover:bg-primary/5 hover:text-primary transition-colors border-primary/20"
        >
          <RefreshCw
            className={`w-3.5 h-3.5 ${loadingConfigs ? "animate-spin" : ""}`}
          />
          Làm mới
        </Button>
      </div>

      <Card className="p-6 shadow-sm border border-border/60 rounded-2xl bg-card">
        <div className="mb-4">
        ...
        <div className="flex items-center gap-4 max-w-md">
          <div className="relative flex-1">
            <Input
              ...
              className="pr-16 text-sm font-medium h-11 rounded-lg border-border/80 bg-card focus-visible:ring-primary/20"
            />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground font-semibold">
              phút
            </div>
          </div>
          <Button
            onClick={() => saveConfig(AppConfigKey.BOOKING_EXPIRY_MINUTES)}
            className="bg-primary hover:bg-primary/95 text-white h-11 px-5 rounded-xl font-semibold cursor-pointer shadow-sm hover:shadow"
          >
            Cập nhật
          </Button>
        </div>
      ...
      <Card className="p-6 shadow-sm border border-border/60 rounded-2xl bg-card">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-12 gap-4 text-[10px] font-bold uppercase tracking-wider text-slate-500 pb-2 border-b border-border/60">
            ...
            >
              <div className="col-span-5 relative">
                <Input
                  ...
                  className="pr-12 h-10 rounded-lg border-border/80 bg-card focus-visible:ring-primary/20"
                />
              ...
              <div className="col-span-5 relative">
                <Input
                  ...
                  className="pr-8 h-10 rounded-lg border-border/80 bg-card focus-visible:ring-primary/20"
                />
                <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">
                  %
                </div>
              ...
              <div className="col-span-2 text-right">
                <Button
                  variant="ghost"
                  size="icon"
                  className="text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg cursor-pointer animate-in fade-in"
                  onClick={() => {
                    const newValue = (
                      appConfigs.find(
                        (c) => c.key === AppConfigKey.CANCELLATION_POLICY,
          ...
          <div className="pt-4 flex justify-between items-center border-t border-border/60">
            <Button
              variant="outline"
              size="sm"
              className="gap-2 rounded-lg cursor-pointer h-9 text-xs bg-card hover:bg-primary/5 hover:text-primary transition-colors border-border/80"
              onClick={() => {
                const currentPolicy =
                  appConfigs.find(
                    (c) => c.key === AppConfigKey.CANCELLATION_POLICY,
            ...
            <Button
              ...
              className="bg-primary hover:bg-primary/95 text-white h-9 px-4 rounded-lg text-xs font-semibold cursor-pointer shadow-sm hover:shadow"
            >
              Lưu chính sách huỷ
            </Button>
          </div>
      ...
      <Card className="p-5 shadow-sm opacity-65 border border-border/60 rounded-2xl bg-card">
        ...
        <div className="space-y-4">
          <div className="p-3 bg-transparent rounded-xl border border-dashed border-border/85 flex justify-between items-center">
            <span className="text-xs font-medium text-slate-600 dark:text-slate-300">
              Cấu hình Email Marketing & Mẫu Gửi Tin
            </span>
            <span className="text-[10px] bg-slate-200 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full font-bold">
              Locked
            </span>
          </div>
        </div>
