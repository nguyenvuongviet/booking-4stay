    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, i) => {
          ...
          return (
            <div
              key={i}
              className={`p-4 sm:p-6 bg-linear-to-br ${stat.color} ${stat.border} border shadow-sm rounded-2xl flex items-center justify-between transition-all hover:scale-[1.02] duration-300`}
            >
              ...
              <div className="p-2.5 sm:p-3 bg-white/80 dark:bg-slate-900/80 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 shrink-0">
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="p-6 rounded-2xl border shadow-sm bg-white dark:bg-slate-900">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4 pb-2 sm:pb-2.5 border-b border-slate-100 dark:border-slate-800">
            ...
            <div className="space-y-2.5 sm:space-y-3">
              {topPromotions.slice(0, 5).map((promo) => {
                ...
                return (
                  <div
                    key={promo.id}
                    className="space-y-1 sm:space-y-1.5 border-b border-slate-50 dark:border-slate-800/30 pb-2 sm:pb-3 last:border-0 last:pb-0"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="font-mono text-[10px] font-bold text-primary bg-primary/10 border border-primary/20 px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0">
                          {promo.code}
                        </span>
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 justify-between sm:justify-end shrink-0 w-full sm:w-auto border-t sm:border-t-0 pt-1 sm:pt-0 border-slate-100 mt-1 sm:mt-0">
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${typeConfig.color}`}
                        >
                          {typeConfig.label}
                        </span>
                        <span className="text-[10px] sm:text-xs font-bold text-slate-850 dark:text-white">
            ...
            <div className="space-y-2 sm:space-y-2.5">
              {byType.map((bt) => {
                ...
                return (
                  <div
                    key={bt.type}
                    className="flex items-center justify-between gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-xl border border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30"
                  >
