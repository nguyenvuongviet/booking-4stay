...
export default function CalendarGrid({
  ...
  const { loading, getPrice, statusMap, bookingMap, updateCalendar } =
    useCalendarPricing({ roomId, defaultPrice, months });

  const { selectedDates, isEditing, handleSelectDate, clearSelection } =
    useCalendarRoom();
  ...
  return (
    <div className="flex flex-row w-full items-start">
      <div
        className={`transition-all duration-300 flex-1  min-w-0 ${selectedDates.length > 0 ? "w-[calc(100%-380px)]" : "w-full "}`}
      >
      ...
      {isEditing && selectedDates.length > 0 && (
        <div className="shrink-0 bg-card sticky top-0 h-screen overflow-y-auto">
          <EditPanel
            ...
            defaultPrice={defaultPrice}
            isSoldOut={
              statusMap.get(firstKey) === "BOOKED" ||
              statusMap.get(firstKey) === "BLOCKED"
            }
