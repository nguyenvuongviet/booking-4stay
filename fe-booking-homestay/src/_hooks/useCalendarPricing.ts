...
export function useCalendarPricing({
  ...
  const statusMap = useMemo(() => {
    ...
    data.forEach((d) => {
      map.set(toDateKey(new Date(d.date + "T00:00:00")), d.status);
    });
  ...
  const bookingMap = useMemo(() => {
    ...
    data.forEach((d) => {
      if (d.bookingDetails) {
        map.set(toDateKey(new Date(d.date + "T00:00:00")), d.bookingDetails);
      }
    });
  ...
  const updateCalendar = async (
    ...
    setData((curr) =>
      curr.map((item) => {
        ...
        const newStatus: CalendarItem["status"] = found.isAvailable
          ? "AVAILABLE"
          : "BLOCKED";
    ...
    payload.forEach((p) => {
      ...
      const updatedMonth = monthData.map((item) => {
        if (toDateKey(new Date(item.date)) !== toDateKey(d)) return item;

        const newStatus: CalendarItem["status"] = p.isAvailable
          ? "AVAILABLE"
          : "BLOCKED";
