...
import { search_location } from "@/services/locationApi";
...
export function SearchBar({ compact = false }: SearchBarProps) {
  ...
  const fetchLocationSuggestions = useCallback(async (query: string) => {
    if (!query.trim()) {
      //input rỗng
      const res = await getProvinces(1);
      const allData = res || [];
  ...
  return (
    <div
      className={`mx-auto w-full bg-card rounded-4xl shadow-lg transition-all duration-300 ${compact
        ? "max-w-2xl p-2 sm:scale-[80%] md:scale-[100%]"
        : "max-w-5xl p-2 sm:scale-[80%] md:scale-[90%] md:p-3"
        }`}
    >
      <div
        className={`${compact ? "flex items-center gap-2" : "grid grid-cols-7 gap-4"
          }`}
      >
